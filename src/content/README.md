...
| `product-detail.html` | Obsah → **Produktový detail** | detail každého produktu | `.pd-benefits` (`24-product-detail.css`) + `.pd-size-src` (`24-size-chart.css`) | ⏳ vkládá uživatel (zdrojový `</>` režim); pás benefitů + **skrytý zdroj tabulky velikostí** (jedna `.pd-size-item` = jedna záložka v okně; sedmá záložka se přidá tady, ne v kódu) + **skrytý zdroj textu sekcí accordionu** `.pd-acc-src` (jedna `.pd-acc-item` = jedna sekce, `h3.pd-acc-name` musí sedět na název sekce v accordionu; zatím „Doprava a vrácení" — ceník dopravy je `<table class="pd-ship">`, aby šla cena přepsat i ve WYSIWYG). Co tu není, jede ze záložního znění v `src/scripts/40-product-detail.js`. ⚠️ Karta „Zvýhodněné balení“ tady NENÍ — jede z nativních **Doplňkových služeb**, viz sekce níže |

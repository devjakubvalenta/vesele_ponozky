...
| `product-detail.html` | Obsah → **Produktový detail** | detail každého produktu | `.pd-benefits` (`24-product-detail.css`) + `.pd-size-src` (`24-size-chart.css`) | ⏳ vkládá uživatel (zdrojový `</>` režim); pás benefitů + **skrytý zdroj tabulky velikostí** (jedna `.pd-size-item` = jedna záložka v okně; sedmá záložka se přidá tady, ne v kódu) + **skrytý zdroj textu sekcí accordionu** `.pd-acc-src` (jedna `.pd-acc-item` = jedna sekce, `h3.pd-acc-name` musí sedět na název sekce v accordionu; „Materiál a péče", „Doprava a vrácení", „Časté dotazy" — ceník dopravy je `<table class="pd-ship">`, aby šla cena přepsat i ve WYSIWYG; okruhy FAQ jsou `<details>`, ty upravovat ve zdrojovém režimu). Co tu není, jede ze záložního znění v `src/scripts/40-product-detail.js`. ⚠️ Karta „Zvýhodněné balení“ tady NENÍ — jede z nativních **Doplňkových služeb**, viz sekce níže |
...
| `email-potvrzeni-objednavky.html` | Nastavení → e-mail **zákazníkovi** o přijetí objednávky (ne notifikace majiteli „V Exitshopu byla vytvořena nová objednávka“) | e-mail zákazníka | jen inline styly (e-mailoví klienti zahazují `<style>`) | ⏳ vkládá uživatel (zdrojový `</>` režim); předmět je v komentáři souboru. Šablona NEMÁ placeholder čísla účtu ani QR — ověřit, jestli platforma posílá platební instrukce zvlášť. ⚠️ Závěrečný odstavec **slibuje další e-mail** s tipy na péči — ten musí vzniknout jako automatizace, jinak je to nesplněný slib |
| `email-newsletter-potvrzeni.html` | Nastavení newsletteru → „Odeslat uživateli email po přihlášení k odběru novinek“ → pole **Text emailu** | e-mail odběratele | jen inline styly | ⏳ vkládá uživatel (zdrojový `</>` režim); předmět je v komentáři souboru. Nese slevový kód **VESELE15** — musí sedět s `footer.js` → `NEWSLETTER.code` **i** s kupónem v administraci. ⚠️ Patička webu používá **Ecomail embed**, ne nativní pole — pokud má Ecomail welcome automatizaci, hrozí **dva uvítací e-maily**; před zapnutím ověřit |

...
| `countdown-source.html` | Obsah → **Box pod hlavičkou** | všechny stránky (blok je skrytý) | `#vp-cd-src` (`src/css/20-header.css`) | ⏳ vkládá uživatel (zdrojový `</>` režim); skrytý `%countdown%` = jediný zdroj konce akce pro odpočet v liště (`src/scripts/countdown-bar.js`) |

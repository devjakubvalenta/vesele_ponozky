...
| `countdown-bar.js` | Odpočet v liště | Na všech stránkách | ne (patička) | ⏳ vlož 1× jako `<script src="…jsDelivr…@<hash>/src/scripts/countdown-bar.js">` — **pin hashem**; styl `src/css/20-header.css` (`.vp-cd`). **Datum konce akce se v kódu NENASTAVUJE** — čte se z administrace (Marketing a slevy → „Odpočítávání na e-shopu“). Podmínka: skrytý blok `%countdown%` z `src/content/countdown-source.html` musí být vložený v „Obsah → Box pod hlavičkou“, jinak lišta zůstane bez odpočtu. ⚠️ Náš span je `.vp-cd`, **ne** `.es-countdown` — platformní `Countdown.js` po doběhnutí dělá `$('.es-countdown').remove()` |
